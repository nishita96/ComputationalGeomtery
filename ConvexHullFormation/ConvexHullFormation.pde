...
int numOfPoints = 10;// CHANGE BACK TO 4
...
void draw(){
  ...
  textRHC( "p: Performance Test", 10, height-120 );
...
void keyPressed(){
  ...
  // if( key == 'g' ){ makeRandomPointsVersion2(); calculateConvexHull(); }
  // if( key == 'g' ){ makeRandomPointsVersion3(); calculateConvexHull(); }
  if( key == 'c' ){ points.clear(); calculateConvexHull(); }
  if( key == 'p' ){ performanceTest(); } // ADDED PERFORMANCE TEST 
}
